...
class UserBehavior(TaskSet):
    ...
    def on_start(self):
        ...
        self.suc_fail_sure = random.choice(suc_fail)
        print("----初始化suc_fail_sure----")
        print(self.suc_fail_sure)
    ...
    # @task(1)
    @seq_task(2)
    def test_charge_msgFeedBack(self):
        # 更新短信状态接口
        headers = {"Content-Type": "application/json"}
        data = {"msgId": self.msgID,
                "status":self.suc_fail_sure,
                "token": self.token}
        r = self.client.post(
            url='/charge/msg_feed_back',
            headers=headers,
        ...
        print("改状态的token", self.token)
        print("改状态的msgID",self.msgID)
        print("改状态的sure",self.suc_fail_sure)
...
if __name__ == '__main__':
    import os
    os.system("locust -f test_charge_msg.py")